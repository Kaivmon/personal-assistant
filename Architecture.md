...
- `personal-assistant` is a local HTTP service for note taking, deterministic retrieval, SQL analytics, and temporary Markdown generation.
- `openclaw-plugin/personal-assistant` is a native OpenClaw tool plugin that registers the `personal_assistant` tool and calls the local HTTP service.
...
- `openclaw-plugin/personal-assistant`: OpenClaw-native plugin package for Discord-facing tool calls.
